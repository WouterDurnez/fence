...
def handler(event, context):
    ...
    chunks = split_text(text=input_text, chunk_size=30000, overlap=.1)

    logger.info(f"🔪 Chunked input text into {len(chunks)} chunks of approximately 10k characters each")
    ...
    for chunk in chunks[:]:
        ...
        faq_results.append(links['faq'].run(input_dict={"state": chunk, "number_of_questions": NUMBER_OF_QUESTIONS})["state"])
    ...
    # Build response
    return {"statusCode": 200, "body": {'summary': meta_summary, 'summaries': summaries, 'question_answers': question_answers}}
