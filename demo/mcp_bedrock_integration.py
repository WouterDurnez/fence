...
def demo_single_mcp_client():
    ...
    with MCPClient(read_timeout_seconds=30) as client:
        try:
            client.connect(
                "stdio",
                command="uvx",
...
def demo_multiple_mcp_clients():
    ...
    with (
        MCPClient(read_timeout_seconds=10) as client1,
        MCPClient(read_timeout_seconds=10) as client2,
    ):
        ...
        try:
            ...
            print(f"❌ Error: {e}")
            return None
        # Clients will be automatically disconnected when exiting the context
...
def demo_mixed_setup():
    ...
    with MCPClient(read_timeout_seconds=10) as client:
        try:
            client.connect(
                "stdio",
                command="uvx",
